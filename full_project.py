...
#################
# INTERACTIVE MAP OF ALL COLLISION FATALITIES SINCE 2016

# Creates a map located at NYC, specifically in Manhattan
m = folium.Map(location=[40.75, -74.125], zoom_start=12)
# Filters data contains fatalities only
df_deaths = df[df['NUMBER OF PERSONS KILLED'] >= 1]
df_deaths = df_deaths[df_deaths['LONGITUDE'].notna()]
...
for lat, lng in zip(df_deaths['LATITUDE'], df_deaths['LONGITUDE']):
    folium.Marker(
        location=[lat, lng],
        icon=None,
    ).add_to(marker)
